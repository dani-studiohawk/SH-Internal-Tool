...
export async function requireAuth(req, res) {
  ...
  try {
    ...
    const users = await sql`
      ...
      FROM users 
      WHERE email = ${session.user.email} AND status = 'active'
    `;
    ...
    session.user.status = users[0].status;
    session.user.department = users[0].department;
...
export function withAuth(handler) {
  return async (req, res) => {
    ...
    // Add session and user to request object for use in handler
    req.session = session
